...
# Define biobanks with specific search terms and founding years
BIOBANK_DEFINITIONS = {
    'UK Biobank': {
        'terms': [
        ...
        'founding_year': 2006  # UK Biobank began recruitment in 2006
    },
    'Million Veteran Program': {
        ...
        'founding_year': 2011  # MVP launched in 2011
    },
    'FinnGen': {
        'terms': [
        ...
        'founding_year': 2017  # FinnGen started in 2017
    },
    'All of Us': {
        'terms': [
        ...
        'founding_year': 2015  # Announced in 2015, launched 2018
    },
    'Estonian Biobank': {
        'terms': [
        ...
        'founding_year': 2000  # Estonian Genome Project started in 2000
    },
    'Genomics England': {
        'terms': [
            'Genomics England', '100,000 Genomes Project', 
            '100000 Genomes Project', '100k Genomes Project',
            'Genomics England Limited', 'GeL data',
            'Genomics England dataset', 'Genomics England cohort',
            'Genomics England Clinical Interpretation Partnership',
            'GeCIP', 'Genomics England research environment',
            'Genomics England biobank', 'Genomics England participants',
            '100,000 Genomes', '100000 Genomes', '100k Genomes'
        ],
        'founding_year': 2013  # Genomics England established in 2013
    }
}

def create_biobank_search_query(biobank_name):
    """Create search query to find papers that mention this biobank"""
    biobank_terms = BIOBANK_DEFINITIONS[biobank_name]['terms']
    biobank_query = ' OR '.join([f'"{term}"[All Fields]' for term in biobank_terms])
    
    # Use founding year as start date
    founding_year = BIOBANK_DEFINITIONS[biobank_name]['founding_year']
    
    # Just biobank name and date range from founding year
    full_query = f'({biobank_query}) AND ("{founding_year}"[PDAT] : "2025"[PDAT])'
...
def search_pubmed_for_biobank(biobank_name):
    ...
    # Get founding year for this biobank
    founding_year = BIOBANK_DEFINITIONS[biobank_name]['founding_year']
    
    # Break down by year from founding year onwards
    years = range(founding_year, 2026)  # From founding year to 2025
        ...
        try:
            ...
            if required_biobanks.issubset(existing_biobanks):
                ...
                for biobank in BIOBANK_DEFINITIONS.keys():
                    if biobank in biobank_counts.index:
                        count = biobank_counts[biobank]
...
def main():
    ...
    print(f"   Date range: From each biobank's founding year to 2025")
    
    print(f"\n📋 Target biobanks (with founding years):")
    for biobank, info in BIOBANK_DEFINITIONS.items():
        print(f"   - {biobank} (founded {info['founding_year']})")
