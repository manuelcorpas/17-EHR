...
USAGE:
    python 03-03-bhem-compute-metrics.py
...
def load_gbd_registry() -> Dict[str, Dict]:
    ...
    if not INPUT_GBD_REGISTRY.exists():
        raise FileNotFoundError(
            ...
            f"Run 03-01-bhem-map-diseases.py first"
        )
...
def load_publications(filepath: Path, ihcc_registry: Dict[str, Dict] = None) -> pd.DataFrame:
    ...
    if 'cohort_id' in df.columns and 'biobank_id' not in df.columns:
        ...
        logger.info("  Mapped cohort_id -> biobank_id")
...
def main():
    print("=" * 70)
    print(f"HEIM-Biobank v2.0 (IHCC): Compute Health Equity Metrics")
    ...
    # Print summary
    print(f"\n" + "=" * 70)
    print(f"📊 HEIM-Biobank v2.0 (IHCC) METRICS SUMMARY")
    print(f"=" * 70)
    ...
    print(f"\n✅ COMPLETE!")
    print(f"\n➡️  Next step: python 03-05-bhem-generate-json.py")
