...
===========================
HEIM-Biobank v1.0: Generate Dashboard JSON Files
...
USAGE:
    python 02-05-bhem-generate-json.py
...
def generate_equity_json(metrics: Dict) -> Dict:
    ...
    for bid, bdata in biobanks.items():
        ...
        if is_gs or region in LMIC_REGIONS:
...
def main():
    print("=" * 70)
    print(f"HEIM-Biobank v1.0: Generate Dashboard JSON Files")
    print("=" * 70)
    
    # Check input files
    if not INPUT_METRICS.exists():
        print(f"\n❌ Input file not found: {INPUT_METRICS}")
        print(f"   Run 02-03-bhem-compute-metrics.py first")
        return
