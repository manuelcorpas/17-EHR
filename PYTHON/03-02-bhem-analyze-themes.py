#!/usr/bin/env python3
"""
03-02-bhem-analyze-themes.py
============================
BHEM Step 2b: Analyze Research Themes (Year range: 2000-2025) and Knowledge Contributions
...
USAGE:
    python 03-02-bhem-analyze-themes.py
"""
...
def main():
    ...
    if not INPUT_FILE.exists():
        ...
        print("   Run 03-00-bhem-fetch-pubmed.py first")
        return
    ...
    if 'year' in df.columns:
        ...
        if filtered_out > 0:
            print(f"   Filtered: {filtered_out:,} outside year range")
        print(f"   {len(df):,} publications in {MIN_YEAR}-{MAX_YEAR}")
    
    # Detect column naming (cohort vs biobank)
    if 'cohort_id' in df.columns:
        id_col = 'cohort_id'
        name_col = 'cohort_name'
        # Rename to biobank_* for compatibility with rest of script
        df['biobank_id'] = df['cohort_id']
        df['biobank_name'] = df.get('cohort_name', df['cohort_id'])
        print(f"   Using IHCC cohort columns (mapped to biobank_*)")
    else:
        id_col = 'biobank_id'
        name_col = 'biobank_name'
    
    print(f"   Cohorts/Biobanks: {df['biobank_id'].nunique()}")
    ...
    print(f"\n✅ STEP 2b COMPLETE!")
    print(f"\n➡️  Next step: python 03-03-bhem-compute-metrics.py")
