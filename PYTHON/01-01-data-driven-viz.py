#!/usr/bin/env python3
"""
Data-Driven Research Gap Visualizations
======================================

Uses your actual research gap data files to create publication-ready visualizations.
Reads directly from the CSV files generated by your gap analysis script.
...
- ANALYSIS/01-00-RESEARCH-GAP-DISCOVERY/biobank_research_effort.csv
...
def load_research_gap_data():
    """Load the research gap analysis data."""
    
    # File paths (corrected paths)
    ...
    biobank_file = 'ANALYSIS/01-00-RESEARCH-GAP-DISCOVERY/biobank_research_effort.csv'
    
    # Check if files exist
    for file in [gap_file, effort_file, biobank_file]:
        if not Path(file).exists():
            print(f"❌ File not found: {file}")
            print("Please ensure you've run the research gap discovery script first.")
    ...
    try:
        ...
        biobank_df = pd.read_csv(biobank_file)
        
        print(f"✅ Data loaded successfully:")
        print(f"   • {len(gap_df)} diseases analyzed")
        print(f"   • {len(effort_df)} disease areas with research")
        print(f"   • {len(biobank_df)} biobank-disease combinations")
        
        return gap_df, effort_df, biobank_df
...
def create_crisis_visualization(gap_df, output_prefix='research_gap'):
    """
    Create the main crisis visualization from your data with improved annotation positioning.
    """
    
    # Create figure
    fig, ax = plt.subplots(figsize=(16, 12))  # Slightly larger for better readability
    ...
    # Annotate critical diseases with smart positioning to avoid overlaps
    critical_diseases = gap_df[critical_mask].nlargest(5, 'dalys_millions')
    
    # Define smart offsets for each disease to avoid overlaps - improved spacing
    smart_offsets = [
        (15, 50),   # First disease: right and higher up
        (-100, 25), # Second disease: further left and up  
        (15, -35),  # Third disease: right and further down
        (-100, -55), # Fourth disease: further left and down
        (60, 20),   # Fifth disease: further right and up
    ]
    ...
    # Custom legend - moved to upper right to avoid conflict with research statistics
    ...
    # Crisis statistics - improved formatting and positioning
...
def create_combined_crisis_equity_visualization(gap_df, output_prefix='research_gap'):
    """
    Create combined visualization showing both crisis and equity analysis.
    """
    
    # Create figure with custom layout: large crisis plot + 4 equity panels
    ...
    # Legend for main plot - moved to upper right to avoid conflict
    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='#d63031', 
               markersize=12, label=f'CRITICAL GAP (score >{critical_threshold})', markeredgecolor='black'),
    ...
    gap_text = (f"📊 RESEARCH PRIORITIES\n"
                f"• {critical_count} diseases with critical gaps\n"
                f"• {critical_dalys:.1f}M DALYs affected\n"
                f"• Only {critical_pubs} total publications\n"
                f"• Efficiency: {critical_pubs/critical_dalys:.2f} studies/M DALYs\n"
...
def main():
    """
    Main function to generate all visualizations from your data.
    """
    print("🎯 Creating Publication-Ready Research Gap Visualizations")
    print("=" * 60)
    
    # Load data
    gap_df, effort_df, biobank_df = load_research_gap_data()
    ...
    print(f"   (Directory created automatically if it doesn't exist)")
    
    # Generate visualizations
    output_prefix = str(output_dir / 'research_gap')
    
    print("\n🔥 Creating combined research gaps & equity visualization...")
    create_combined_crisis_equity_visualization(gap_df, output_prefix)
    
    print(f"\n✅ VISUALIZATION COMPLETE!")
    print(f"📂 Files saved in: ANALYSIS/01-01-DATA-VIZ/")
    print(f"   • research_gap_priority_areas_equity.png/pdf")
    ...
    print(f"   • Specific research investment recommendations")
    print(f"   • Comprehensive priority areas and equity analysis in one figure")
